...
public class NowplayingHandler
{
    ...
    public void updateTopic(long guildId, AudioHandler handler, boolean wait)
    {
        ...
        if (guild.getSelfMember().getVoiceState().inAudioChannel()) {
            AudioChannelUnion vc = guild.getSelfMember().getVoiceState().getChannel();
            if (guild.getSelfMember().hasPermission((GuildChannel)vc, Permission.MANAGE_CHANNEL)) {
                vc.asVoiceChannel().modifyStatus((handler.getPlayer().isPaused() ? "\u23f8" : "\u266b") + handler.getCurrentTrack(this.bot.getJDA())).queue();
            }
        }
        Settings settings = bot.getSettingsManager().getSettings(guildId);
        final MessageChannel chan = settings.getTextChannel(guild);
        if(chan == null)
            return;
        if(!(chan instanceof TextChannel))
            return;
        final TextChannel tchan = (TextChannel)chan;
        if(guild.getSelfMember().hasPermission(tchan, Permission.MANAGE_CHANNEL))
        {
            String otherText;
    ...
    public void onTrackUpdate(long guildId, AudioTrack track, AudioHandler handler)
    {
        ...
        this.updateTopic(guildId, handler, false);
    }
