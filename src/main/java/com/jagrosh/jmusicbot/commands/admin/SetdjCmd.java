...
public class SetdjCmd extends AdminCommand {
    public SetdjCmd(Bot bot) {
        ...
        this.options = Collections.singletonList(
                new OptionData(OptionType.ROLE, "dj-role", "Role to set as DJ-Role. Omit to clear")
                        .setRequired(false)
        );
    ...
    protected void execute(SlashCommandEvent event) {
        ...
        if (!event.hasOption("dj-role")) {
            s.setDJRole(null);
            event.reply(event.getClient().getSuccess() + " DJ role cleared; Only Admins can use the DJ commands.");
        } else {
            s.setDJRole(event.getOption("dj-role").getAsRole());
            event.reply(event.getClient().getSuccess() + " DJ commands can now be used by users with the **" + event.getOption("dj-role").getAsRole().getName() + "** role.").queue();
