...
public class SkipratioCmd extends AdminCommand {
    public SkipratioCmd(Bot bot) {
        ...
        this.options = Collections.singletonList(
                new OptionData(OptionType.INTEGER, "ratio", "Skip-ratio in percent")
                        .setRequired(true).setRequiredRange(0, 100)
        );
    ...
    protected void execute(SlashCommandEvent event) {
        ...
        int val = event.getOption("ratio").getAsInt();
        if (val < 0 || val > 100) {
            event.reply("The provided value must be between 0 and 100!").setEphemeral(true).queue();
            return;
        }
        Settings s = event.getClient().getSettingsFor(event.getGuild());
        s.setSkipRatio(val / 100.0);
        event.reply("Skip percentage has been set to `" + val + "%` of listeners on *" + event.getGuild().getName() + "*").setEphemeral(true).queue();
